...
class toolsHub {
    constructor(databaseUrl, licenceUrl, apiUrl, client) {
    ...
    start() {
        (0, console_1.logWithLabel)('custom', 'Starting the toolsHub class monitoring system...', 'ETERNAL');
        this.DB();
    }
    async DB() {
        try {
            const connection = await (0, mongoose_1.connect)(this.databaseUrl);
